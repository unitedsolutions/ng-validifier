...
export default (control, configs, validatorDeclaration) => {
  ...
  errorConfigs.asyncDebounceTimeout = setTimeout(() => {
    ...
    validator(control, configs.validator).then(validationStatus => {
      if(errorConfigs.requests[validationId]) {
        return;
      }

      validationStatus = validationStatusNormalizer(validationStatus);
      finalDataSetter(control, validatorDeclaration, validationStatus);
      finalVisualizer(control, validatorName, validationStatus);      
    });
